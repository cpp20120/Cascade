...
```cpp
#include <cascade/pool.hpp>
#include <cascade/scope.hpp>

// Basic usage
cascade::CascadePool pool;
cascade::TaskScope scope(pool);

// Build and execute a DAG
auto task_a = scope.submit([]{ /* task A */ });
auto task_b = scope.then(task_a, []{ /* task B */ });
auto task_c = scope.when_all({task_a, task_b}, []{ /* task C */ });

scope.run_and_wait();

// Parallel for with range
cascade::parallel_for(0, 1000, [](int i) {
    // process element i
});

// Task groups for synchronization
cascade::Quarry quarry;
quarry.submit([]{ /* task 1 */ });
quarry.submit([]{ /* task 2 */ });
quarry.wait();